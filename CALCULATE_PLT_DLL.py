...
#********************************************************************************************
def get_lims_gui(vmp):
    master = ttk.Tk()
#     def get_entry_fields():
#         print("xlim1: %s\nxlim2: %s" % (e1.get(), e2.get()))    
    ttk.Label(master, text = "xlim1 min {:3.1f} or -1".format(vmp[0])).grid(row = 0)
    ttk.Label(master, text = "xlim2 max {:3.1f} or -1".format(vmp[-1])).grid(row = 1)
    e1 = ttk.Entry(master)
    e2 = ttk.Entry(master)
...
def correct_phase_indexes(Lx2, xph_pp, npm_pp, el_names2):
    ...
    FCCs = []
    for ph in xph_pp_corrected.keys():
        if ('FCC' in ph) and not(ph in(BCCs)):
            FCCs.append(ph)
    if len(BCCs) > 0:
        flag = True
        for ph in BCCs:
